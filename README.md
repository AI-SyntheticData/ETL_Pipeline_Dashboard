...
**Solution:**
```bash
pip install "numpy==1.26.4" "shap<0.46" "scikit-learn<1.6"
```
...
**Solution:** Already fixed. If persists:
```bash
...
python3 start_web_dashboard.py --port 8081
```

### Issue: Database Connection Failed

**Cause:** Missing or invalid SUPABASE_KEY

**Solution:**
```bash
export SUPABASE_KEY="your_valid_key"
python3 run_complete_pipeline.py
```

### Issue: LIME Pickle Error

**Error:** `AttributeError: Can't get local object 'BaseDiscretizer.__init__.<locals>.<lambda>'`

**Cause:** LIME explainer contains lambda functions that can't be pickled

**Solution:** Already fixed. The model now:
- Saves without LIME explainer
- Recreates LIME explainer on load if needed
```bash
# Just run normally
python3 train_model.py
```

### Issue: No Dashboards Generated

**Check:**
1. Pipeline completed successfully
2. Output directory exists: `output/YYYYMMDD/`
